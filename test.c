...
char* test_array[5] = {"MORSE", "TEST", "LINUX EMBEDDED", "ROOM2", "FIFTH ELEMENT"};
char* codes[5] = {"--#---#.-.#...#.", "-#.#...#-",".-..#..#-.#..-#-..- .#--#-...#.#-..#-..#.#-..", ".-.#---#---#--#..---","..-.#..#..-.#-#.... .#.-..#.#--#.#-.#-"};
char* output_array;
sem_t shared;
sem_t synchro;
...
void* task_foo(void* p)
{
    while(1)
    {
	   ...
	   /* Init temp buffer. */
	    ...
	    printf("Driver has returned message: %s", output_array);
	    printf("ret_val: %d\n", ret_val);
...
int main(int argc, char*argv[])
{
    ...
    random_array = malloc(MAX_LEN * sizeof(char));
    ...
    pthread_create(&user_communication, NULL, user_comm_foo, NULL);
    pthread_create(&task, NULL, task_foo, NULL);
